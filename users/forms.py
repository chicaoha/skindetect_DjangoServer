...
class ProfileForm(forms.ModelForm): 
    ...
    gender = forms.ChoiceField(label='Gender', choices=[('M', 'Male'), ('F', 'Female'), ('O', 'Other')], required=False)
    address = forms.CharField(max_length=255, label='Address', required=False)
    phone = forms.CharField(max_length=10, label='Phone', required=False, widget=forms.TextInput(attrs={'type': 'tel'}))
